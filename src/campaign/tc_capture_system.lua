...
--
-- Current focus:
-- CaptureSystem v0.2.2 keeps the proven v0.2.1 capture pressure model
-- and automatically consumes completed MissionGenerator v0.2.3 mission
-- effects into state-only capture pressure during capture status updates.
--
-- Version:
-- 0.2.2
--
...
-- - expose Capture Ready and Pressure Contested zones to F10
...
CaptureSystem.lastCompletedMissionEffectSummary = nil
CaptureSystem.processingMissionEffects = false
...
local function ensureCampaignTables()
  ...
  state.Campaign.capture.thresholds.capture =
    state.Campaign.capture.thresholds.capture or CaptureSystem.defaultCaptureThreshold

  state.Campaign.capture.thresholds.contested =
    state.Campaign.capture.thresholds.contested or CaptureSystem.defaultContestedThreshold

  state.Campaign.capture.thresholds.decayPerUpdate =
    state.Campaign.capture.thresholds.decayPerUpdate or CaptureSystem.defaultDecayPerUpdate
...
local function getZoneClass(record)
  ...
  return record.zoneClass
    or record.recommendedZoneClass
...
local function isStrategicOrSecondaryZone(record)
  ...
  if zoneClass == CaptureSystem.captureClasses.MISSION_EDITOR_CAPTURE_ZONE then
    return true
  end
...
local function getLinkedBaseFromZone(zoneRecord)
  ...
  if zoneRecord.linkedAirbaseKey == nil
    and zoneRecord.linkedAirbaseName == nil
    and zoneRecord.baseKey == nil
    and zoneRecord.airbaseKey == nil then
    return nil, nil
  end

  local registry = getBaseRegistry()

  local linkedKey = zoneRecord.linkedAirbaseKey
    or zoneRecord.baseKey
    or zoneRecord.airbaseKey

  if linkedKey ~= nil and registry[linkedKey] ~= nil then
    return registry[linkedKey], linkedKey
  end

  local linkedName = zoneRecord.linkedAirbaseName
    or zoneRecord.baseName
    or zoneRecord.airbaseName

  if linkedName ~= nil then
    return findRecordByKeyOrName(registry, linkedName)
  end
...
local function getLinkedZonesFromBase(baseRecord)
  ...
  for key, zoneRecord in pairs(getZoneRegistry()) do
    if zoneRecord.linkedAirbaseKey == baseRecord.key
      or zoneRecord.baseKey == baseRecord.key
      or zoneRecord.airbaseKey == baseRecord.key then
...
local function getMissionTargetZoneKey(missionRecord)
  ...
  if type(missionRecord.effectState) == "table" then
    if missionRecord.effectState.targetZoneKey ~= nil then
      return missionRecord.effectState.targetZoneKey
    end
    if type(missionRecord.effectState.capture) == "table"
      and missionRecord.effectState.capture.targetZoneKey ~= nil then
      return missionRecord.effectState.capture.targetZoneKey
    end
  end

  local targetBaseKey = missionRecord.targetBaseKey
  if targetBaseKey == nil and type(missionRecord.effectState) == "table" then
    targetBaseKey = missionRecord.effectState.targetBaseKey
  end

  if targetBaseKey ~= nil then
    for key, zoneRecord in pairs(getZoneRegistry()) do
      if zoneRecord.linkedAirbaseKey == targetBaseKey
        or zoneRecord.baseKey == targetBaseKey
        or zoneRecord.airbaseKey == targetBaseKey then
        return key
      end
    end
  end
...
local function getEffectTable(missionRecord)
  if type(missionRecord) ~= "table" then
    return {}
  end

  if type(missionRecord.effectState) == "table" then
    if type(missionRecord.effectState.rawEffect) == "table" then
      return missionRecord.effectState.rawEffect
    end
  end

  if type(missionRecord.effect) == "table" then
    return missionRecord.effect
  end
...
local function getCapturePressureFromMission(missionRecord)
  ...
  local effect = getEffectTable(missionRecord)
  local pressure = 0

  if type(missionRecord.effectState) == "table"
    and type(missionRecord.effectState.capture) == "table"
    and type(missionRecord.effectState.capture.pressure) == "number" then
    pressure = pressure + missionRecord.effectState.capture.pressure
  end

  if type(effect.capturePressure) == "number" then
    pressure = pressure + effect.capturePressure
  end
  if type(effect.captureProgress) == "number" then
    pressure = pressure + effect.captureProgress
  end
  ...
  if pressure <= 0 then
    ...
    elseif missionRecord.type == "CAS" then
      pressure = 15
    elseif missionRecord.type == "SEAD"
      or missionRecord.type == "DEAD"
      or missionRecord.type == "IADS_SUPPRESSION" then
      pressure = 10
    elseif missionRecord.type == "INTERDICTION" then
      pressure = 10
    elseif missionRecord.type == "RECON" then
      pressure = 5
    elseif missionRecord.type == "FOB_SUPPORT"
      or missionRecord.type == "LOGISTICS" then
      pressure = 5
    end
  end
...
local function getMissionPressureOwner(missionRecord, options)
  local applyOptions = options or {}

  if applyOptions.owner ~= nil and isValidOwner(applyOptions.owner) == true then
    return applyOptions.owner
  end

  if type(missionRecord) == "table" then
    if missionRecord.playerCoalitionOwner ~= nil and isValidOwner(missionRecord.playerCoalitionOwner) == true then
      return missionRecord.playerCoalitionOwner
    end
    if missionRecord.requestingOwner ~= nil and isValidOwner(missionRecord.requestingOwner) == true then
      return missionRecord.requestingOwner
    end
    if missionRecord.executingOwner ~= nil and isValidOwner(missionRecord.executingOwner) == true then
      return missionRecord.executingOwner
    end
  end
...
local function registerCompletionHook(hookData)
  ...
  state.Campaign.capture.completionHooks[hookKey] = {
    ...
    createdAt = hookData.createdAt or getCurrentTime(),
    updatedAt = getCurrentTime()
  }
...
local function findMissionByKey(missionKey)
  ...
  if missionGenerator == nil or missionKey == nil then
    return nil
  end

  if missionGenerator.getMissionByKey ~= nil then
    local missionRecord = missionGenerator.getMissionByKey(missionKey)
    if type(missionRecord) == "table" then
      return missionRecord
    end
  end

  if missionGenerator.getMission ~= nil then
    local missionRecord = missionGenerator.getMission(missionKey)
    if type(missionRecord) == "table" then
      return missionRecord
    end
  end
  ...
  if missionGenerator.getAvailableMissions ~= nil then
    table.insert(containers, missionGenerator.getAvailableMissions())
  end
  if missionGenerator.getActiveMissions ~= nil then
    table.insert(containers, missionGenerator.getActiveMissions())
  end
  if missionGenerator.getCompletedMissions ~= nil then
    table.insert(containers, missionGenerator.getCompletedMissions())
  end
  if missionGenerator.getFailedMissions ~= nil then
    table.insert(containers, missionGenerator.getFailedMissions())
  end
  ...
  for _, container in ipairs(containers) do
    if type(container) == "table" then
      ...
      for _, missionRecord in pairs(container) do
        if type(missionRecord) == "table" and missionRecord.key == missionKey then
          return missionRecord
        end
      end
    end
...
function CaptureSystem.applyMissionEffect(missionKeyOrRecord, options)
  ...
  if type(missionKeyOrRecord) == "table" then
    ...
    missionRecord = findMissionByKey(missionKeyOrRecord)
  end
  ...
  local applyOptions = options or {}
  local owner = getMissionPressureOwner(missionRecord, applyOptions)
  ...
  if type(missionRecord.effectState) == "table" then
    missionRecord.effectState.status = "APPLIED_STATE_ONLY"
    missionRecord.effectState.applied = true
    missionRecord.effectState.appliedAt = getCurrentTime()
    missionRecord.effectState.stateOnly = true
    missionRecord.effectState.capture = missionRecord.effectState.capture or {}
    missionRecord.effectState.capture.status = "APPLIED_STATE_ONLY"
    missionRecord.effectState.capture.applied = true
    missionRecord.effectState.capture.targetZoneKey = zoneKey
    missionRecord.effectState.capture.pressure = amount
  end
...
function CaptureSystem.applyCompletedMissionEffects(options)
  ...
  if CaptureSystem.processingMissionEffects == true then
    return true, CaptureSystem.lastCompletedMissionEffectSummary or {
      applied = 0,
      skipped = 0,
      failed = 0,
      recursiveGuard = true
  ...
  CaptureSystem.processingMissionEffects = true
  ...
  local applyOptions = options or {}
  local safeOptions = {
    owner = applyOptions.owner or getOwnerBlue(),
    autoEvaluate = applyOptions.autoEvaluate == true,
    autoCapture = applyOptions.autoCapture == true,
    reason = applyOptions.reason or "completed_mission_effect_applied_to_capture"
  }

  for _, missionRecord in pairs(completedMissions) do
    if type(missionRecord) == "table" then
      local success, resultOrReason = CaptureSystem.applyMissionEffect(missionRecord, safeOptions)

      if success == true then
        applied = applied + 1
      elseif resultOrReason == "mission_effect_already_applied" then
        skipped = skipped + 1
      else
        failed = failed + 1
  ...
  CaptureSystem.lastCompletedMissionEffectSummary = {
    applied = applied,
    skipped = skipped,
    failed = failed,
  ...
  CaptureSystem.lastMissionEffectSummary = CaptureSystem.lastCompletedMissionEffectSummary
  ...
  logInfo(
    "Completed mission effects processed: applied="
      ...
      .. tostring(CaptureSystem.lastCompletedMissionEffectSummary.appliedMissionEffects)
  )

  return true, CaptureSystem.lastCompletedMissionEffectSummary
end
...
function CaptureSystem.updateCaptureProgress(options)
  ...
  if updateOptions.processMissionEffects ~= false then
    CaptureSystem.applyCompletedMissionEffects({
      owner = getOwnerBlue(),
      autoEvaluate = false,
      autoCapture = false,
      reason = "capture_progress_update_completed_mission_effects"
    })
  end
  ...
  CaptureSystem.lastPressureSummary = {
    ...
    appliedMissionEffects = countTableKeys(state.Campaign.capture.appliedMissionEffects),
    lastMissionEffectSummary = copyValue(CaptureSystem.lastCompletedMissionEffectSummary),
    updatedAt = getCurrentTime()
  }
  ...
  logInfo(
    "Capture progress updated: zones="
      ...
      .. tostring(contested)
      .. ", appliedMissionEffects="
      .. tostring(state.Campaign.capture.statistics.appliedMissionEffects)
  )
...
function CaptureSystem.getBasesByOwner(owner, options)
  ...
  for key, record in pairs(getBaseRegistry()) do
...
function CaptureSystem.getZonesByOwner(owner, options)
  ...
  for key, record in pairs(getZoneRegistry()) do
    local recordOwner = getRecordOwner(record)
...
function CaptureSystem.getCaptureSummary()
  ...
  return {
    ...
    appliedMissionEffects = pressureSummary.appliedMissionEffects or summary.appliedMissionEffects or 0,
...
function CaptureSystem.summary()
  ...
  return {
    ...
    lastCompletedMissionEffectSummary = CaptureSystem.lastCompletedMissionEffectSummary,
    bases = bases,
    zones = zones,
    capture = capture
  }
